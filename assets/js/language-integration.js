...
class LanguageIntegrationController {
    ...
    initializeElements() {
        ...
        if (!this.languageSelector || !this.languageToggle || !this.languageDropdown) {
            // If elements are missing, it might be because ensureLanguageBand skipped injection
            // In that case, we might need to find the nav toggle instead
            const navToggle = document.querySelector('.nav-language-toggle');
            if (navToggle) {
                // Adapt to nav toggle if needed, or just log warning
                console.warn('Using existing nav toggle instead of injected band');
                return;
            }
            // If we really expect them and they are missing, throw error
            // But if we skipped injection, they won't be there. 
            // So let's be graceful.
            console.log('Language band elements not found (possibly intentionally skipped).');
        }
    }

    static ensureLanguageBand() {
        // Check if language band is explicitly disabled
        if (document.body.classList.contains('no-language-band')) {
            return;
        }

        const existingBand = document.querySelector('.language-band');
        if (!existingBand) {
            this.injectLanguageBand();
        }
    }

    static injectLanguageBand() {
